...
class App extends Component {
  ...
  componentDidMount(){
    ...
    let getpetsmart =  new Promise ((resolve, reject) => {
    ...
    let getrd = new Promise ((resolve, reject) => {
    ...
    let gethss = new Promise ((resolve, reject) => {
    ...
    Promise.allSettled([getpetsmart, getrd, gethss])
    .then((values) => {
      console.log("PromiseAllSettledResult: " + JSON.stringify(values))
      if(values[0].status === "fulfilled" || values[1].status === "fulfilled" || values[2].status === "fulfilled"){
        this.setState({dogfetchcomplete: true, promisesettledvalue: values})
      }
    })
    .catch((e) => {
      console.log(e)
  ...
  // handler for both search buttons on homepage
  goSearch = () => {
